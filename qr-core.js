...
class QRCodeCore {
    constructor() {
        ...
        this.MODE_NUMBER = 1 << 0;
        this.MODE_ALPHA_NUM = 1 << 1;
        this.MODE_8BIT_BYTE = 1 << 2;
        this.MODE_KANJI = 1 << 3;
    }

    // QR Code generation
    generateQR(text, errorCorrectionLevel = this.ERROR_CORRECT_M) {
        const qr = new QRCodeModel(-1, errorCorrectionLevel);
        qr.addData(text);
        qr.make();
        return qr;
    }

    // Convert QR model to canvas
    drawOnCanvas(canvas, qrModel, options = {}) {
        const {
            cellSize = 4,
            margin = 4,
            darkColor = '#000000',
...
class QRCodeModel {
    ...
    make() {
        // Auto-detect type number if not set
        if (this.typeNumber == -1) {
            this.typeNumber = this.getTypeNumber();
        }
    ...
    setupPositionAdjustPattern() {
        ...
        if (!pos || pos.length === 0) {
            return; // No position adjustment pattern for this type number
        }
...
class QRUtil {
    ...
    static getPatternPosition(typeNumber) {
        if (typeNumber < 1 || typeNumber > QRUtil.PATTERN_POSITION_TABLE.length) {
            return [];
        }
